...
sub set_up {
    ...
    $this->{db} = new Foswiki::Plugins::FormQueryPlugin::WebDB(
        $this->{test_web} );
...
sub test_tables {
    ...
    if (0 && $Foswiki::cfg{Plugins}{SpreadSheetPlugin}{Enabled}) {
        ...
        $res = Foswiki::Plugins::FormQueryPlugin::WebDB::formQueryOnQuery(
...
sub test_sumQuery {
    ...
    $this->assert_str_equals("", $res);
    my $sum = Foswiki::Plugins::FormQueryPlugin::WebDB::sumQuery('fred', "Size");
...
sub test_tableFormat {
    my $this = shift;
    my $db = $this->{db};
    my $attrs = new Foswiki::Attrs( "header=\"| *Name* | *Parent* |\" format=\"|\$topic|\$subdir_of.name|\" sort=\"topic\"" );
    my $res = Foswiki::Plugins::FormQueryPlugin::WebDB::tableFormat("TF", "|\$RealName|\$Level|", $attrs);
    $this->assert_str_equals("", $res);
    $res = $db->formQueryOnDB('fred', "name='.*'");
    my $qr = $db->query('fred');
    $attrs = new Foswiki::Attrs( "format=\"TF\"" );
    $res = Foswiki::Plugins::FormQueryPlugin::WebDB::showQuery('fred', "TF", $attrs);
...
sub test_fieldSum {
    ...
    $res = Foswiki::Plugins::FormQueryPlugin::WebDB::tableFormat("TF", "\$FileTable.Size", $attrs);
    $this->assert_str_equals("", $res);
    my $qr = $db->query('fred');
    $attrs = new Foswiki::Attrs( "format=\"TF\"" );
    $res = Foswiki::Plugins::FormQueryPlugin::WebDB::showQuery('fred', "TF", $attrs);
...
sub test_more_fqp {
    ...
    my $res = Foswiki::Plugins::FormQueryPlugin::WebDB::tableFormat(
        "\$web.\$topic ", $attrs);
...
sub test_calc {
    ...
    my $qr = $db->query('q3');
    $this->assert_equals(11, $qr->size());
    my $attrs = new Foswiki::Attrs( "format=\"|[[\$_up.web.\$_up.topic]]|\$Name|\$Date|\" sort=\"Name\"" );
    $res = Foswiki::Plugins::FormQueryPlugin::WebDB::toTable('q3', "|[[\$_up.web.\$_up.topic]]|\$Name|\$Date|\" sort=\"Name\"", $attrs, "WebHome", "Test", "WikiGuest", "Foswiki");
    $this->assert_str_equals("", $res);
    ...
    $attrs = new Foswiki::Attrs( "format=\"|[[\$_up.web.\$_up.name]]|\$Level|\$RealName|\"" );
    $res = Foswiki::Plugins::FormQueryPlugin::WebDB::toTable('q3', "\"|[[\$_up.web.\$_up.name]]|\$Level|\$RealName|\"", $attrs, "WebHome", "Test", "WikiGuest", "Foswiki");
    $this->assert_str_equals("", $res);
    #  $res = Foswiki::Plugins::FormQueryPlugin::_handleCalcTable("SHOWCALC", "\$ROW()");
    ...
    #$res = Foswiki::Plugins::FormQueryPlugin::WebDB::showQuery('q3', "\"|%CALC{\\\"\$dollarROW()\\\"}\$percnt()|[[\$_up.web.\$_up.name]]|\$Level|\$RealName|\"", $attrs, "Test", $this->{test_web}, $this->{twiki}->{user});
    #print $res;
    #print Foswiki::Func::getRenderedVersion($res);
...
sub bmFn {
    ...
    Foswiki::Plugins::FormQueryPlugin::WebDB::formQueryOnQuery("q2", "Type='text'", "q1");
}
